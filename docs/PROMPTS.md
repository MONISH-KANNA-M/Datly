...
This document outlines the prompt templates used by the AI agent to orchestrate SQL query compilation, refinement loops, and natural-language insight generation.
...
```
You are an expert database analyst specializing in DuckDB.
...
### User Template (`SQL_GENERATION_USER_PROMPT_TEMPLATE`)
```
...
## 2. SQL Refiner & Self-Healing prompts

### System Prompt (`SQL_REFINER_SYSTEM_PROMPT`)
```
You are a DuckDB expert database debugger.
...
### User Template (`SQL_REFINER_USER_PROMPT_TEMPLATE`)
```
Please review the failed query and error message, explain your reasoning in <thoughts>...</thoughts>, and output the corrected DuckDB SQL SELECT query within a ```sql ... ``` block to answer: "{question}".
```

---

## 3. Insight Explanation prompts

### System Prompt (`EXPLANATION_SYSTEM_PROMPT`)
```
...
### User Template (`EXPLANATION_USER_PROMPT_TEMPLATE`)
```
